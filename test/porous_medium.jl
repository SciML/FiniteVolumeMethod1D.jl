...
λ = t -> (t / t₀)^(1 / (2 + m))
exact_solution = (
    x, t) -> abs(x) > r₀ * λ(t) ? zero(x) : u₀ / λ(t) * (1 - (x / (r₀ * λ(t)))^2)^(1 / m)
L = 15.0
Q = 1.0
N = 2001
...
ax = Axis(
    fig[1, 1], xlabel = L"x", ylabel = L"u(x)", title = L"(a):$ $ Numerical solution",
...
ylims!(ax, -1e-6, 5)
xlims!(ax, -1, 1)
ax = Axis(fig[1, 2], xlabel = L"x", ylabel = L"u(x)", title = L"(b):$ $ Exact solution",
    titlealign = :left, width = 600, height = 300)
[lines!(ax, mesh_points, exact[i], color = colors[i]) for i in eachindex(sol)]
ylims!(ax, -1e-6, 5)
xlims!(ax, -1, 1)
ax = Axis(fig[1, 3], xlabel = L"x", ylabel = L"u(x)", title = L"(c):$ $ Error",
    titlealign = :left, width = 600, height = 300)
...
let t_range = LinRange(0.0, final_time, 250)
    ...
    surface!(ax, mesh_points[in_int_range], t_range,
        reduce(hcat, sol_u)[in_int_range, :], colormap = :viridis)
    xlims!(ax, -1, 1)
