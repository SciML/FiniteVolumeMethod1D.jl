...
@test_throws ArgumentError FVM.get_ab(dirc, 0.5, 0.3)
...
@test FVM.get_ab(neum, 0.5, 0.25) == (-f(0.5,0.25, p), 1.0)
...
@test FVM.get_ab(robin, 0.5, -3.0) == (-2.5, 3.0)
@test robin(0.881, 0.119) == (1.0, 3.0)
@inferred robin(0.881, 0.119)
@test Robin(f) == Robin(f, nothing)
robin = Robin(0.5, 0.3)
@test robin(0.5,0.2) == (0.5, 0.3)
@inferred robin(0.5,0.991)
robin = Robin(0, 3)
@test robin(0,0.2) == (0, 3) && robin(0,0.2) isa Tuple{Int, Int}
@test robin(0.3,0.2) == (0.0, 3.0) && robin(0.3,0.2) isa Tuple{Float64, Float64}
@inferred robin(0,0.2)
@inferred robin(0.0,0.2)
